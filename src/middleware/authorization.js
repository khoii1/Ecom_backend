...
export function authorizeByRoles(allowed = []) {
  return (req, res, next) => {
    if (!req.currentUser) return res.status(401).json({ message: 'Không có quyền truy cập' });
...
export function authorizeStoreOwnership(paramName = 'storeId') {
  return async (req, res, next) => {
    ...
    const r = await databasePool.query('SELECT 1 FROM stores WHERE id=$1 AND owner_id=$2', [storeId, req.currentUser?.id]);
...
export function authorizeSelfOrAdmin(getUserId) {
  return (req, res, next) => {
    ...
    if (!req.currentUser) return res.status(401).json({ message: 'Không có quyền truy cập' });
    if (req.currentUser.role === ROLES.ADMIN) return next();
    if (req.currentUser.id === targetUserId) return next();
    return res.status(403).json({ message: 'Không được phép' });
  };
}
