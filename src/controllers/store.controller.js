...
export const StoreController = {
  ...
  detail: handle(async (req, res) => {
    ...
    if (!data) return res.status(404).json({ message: 'Cửa hàng không tồn tại' });
    res.json(data);
  }),
  ...
  update: handle(async (req, res) => {
    try {
      const updated = await StoreService.update(req.currentUser || {}, req.params.storeId, req.body);
      if (!updated) return res.status(404).json({ message: 'Cửa hàng không tồn tại' });
      res.json(updated);
    } catch (e) {
